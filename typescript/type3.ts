...
const book: Book = {name: '타입 스크립트', author: '마인크로소프트'};
...
/* 타입 가드 함수 (is 사용) */
// - 두 개 이상의 타입이 같은 프라퍼티를 가지고 있으면 in 연산자로
//    타입 가드를 할 수 없으므로 타입가드 함수를 사용해야 함.
...
/* 구별된 유니온 타입 (discriminated unions) */
// 두 개 이상의 타입이 같은 이름의 프라퍼티만 가지고 있을 때 프라퍼티의 값으로 타입 가드
interface Designer {
    name: string,
    age: number,
    jobName: 'designer'
