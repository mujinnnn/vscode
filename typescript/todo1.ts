...
function registTodo(paramTodo: Todo): void {
    // id가 존재하면 todo 배열에 새로운 todo를 삽입
...
function updateTodo(paramTodo: Todo): Todo[] {
    ...
    // id에 해당하는 todo가 존재하면
    if (isExistedTodo(id)) {
        // id에 해당하는 todo가 삭제된 배열과 수정할 데이터가 담긴 todo를 합친 새로운 배열을 리턴
