...
/* array */
let arr1: string[] = ['홍길동', '강감찬', '이순신']; // 문자열 배열 타입
let arr2: Array<string> = ['홍길동', '강감찬', '이순신']; // 제네릭
...
/* any */
// 어떤 타입 값도 모두 허용
// any 타입을 많이 쓰면 타입 스크립트를 사용할 이유가 없다.
...
/* function */
// 파라미터, 반환타입을 지정
function getStr(str: string): string { // 파라미터 - (str: string), 반환타입 - : string
    return 'hi ' + str;
}
getStr('홍길동');
