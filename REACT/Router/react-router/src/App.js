// React Router DOM
// - 경로별 컨텐츠 또는 페이지를 설정할 때 사용
// - BrowserRouter 컴포넌트 또는 HashRouter 컴포넌트로 App 컴포넌트를 감싸줌
// - BrowserRouter : 웹 브라우저 기반의 라우터
// - HashRouter : 웹 서버가 다른 URL 호출 시 동일한 페이지를 보여줄 수 없는 경우에만 사용 (주소 앞에 /#/이 붙음)
// - Routes 컴포넌트 : Route들의 모임
// - Route 컴포넌트 : path(경로)와 element(컴포넌트)
// - Link 컴포넌트 : to(경로), a엘리먼트는 페이지가 리로딩되므로 Link 컴포넌트를 사용해서 리로딩을 방지
// - NavLink 컴포넌트 : Link의 업그레이드 버전, 만들어질 때 현재 경로에 해당하는 NavLink의 class 속성이 active로 설정됨
// - useParams 훅 : 라우팅 시 파라미터들의 정보를 가진 훅
