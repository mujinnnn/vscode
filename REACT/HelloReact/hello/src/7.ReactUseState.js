// 스테이트 (State)
// - 컴포넌트 내부의 데이터
// - 스테이트의 값(value)과 값 변경자(setter)로 구성
// - prop은 컴포넌트 내부로 외부 컴포넌트의 속성 값을 가져오기 위해 사용되고
//  스테이트는 컴포넌트 내부의 값을 저장하고 값의 변경에 따라서 UI가 실시간 자동으로 변경되게 하기 위해 사용됨 (**중요**)

import {useState} from 'react';

export default function ReactuseState() {

    // value가 count이고 setter가 setCount인 스테이트, 기본값이 0
    ...
    return (
        <div>
            ...
            <h2>4. 데이터 가져오기</h2>
            <p>
                <button onClick={changeData}>전체 데이터</button>
            </p>
            <p>
