...
// 클로져(closure) 생성 시 주로 사용
function outer() {
    inner(); // inner 함수가 outer 함수 블록 상단에 호이스팅 됨 (선언이 상단으로 끌어올려짐)
    const o = 'o';
    console.log(o);
    function inner() {
        const i = 'i';
...
//                   함수 상호 간의 독립성을 유지할 수 있으며 
//                   프로그램 전체를 유연하고 확장성있게 구성할 수 있음.
// 고차(higher order) 함수 : 콜백 함수를 전달받은 함수
...
// cf) 문자의 아스키코드값 : console.log('a'.charCodeAt(0));
