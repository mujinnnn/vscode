...
m : 문자열이 여러 라인일 때 여러 라인에서 모두 검색 (multi line)
*/

// 정규표현식 메소드

// 1. RegExp 객체 메소드
// test() : 정규표현식에 일치하는 문자열이 있는 지 true/false로 반환
// exec() : 정규표현식에 일치하는 문자열을 반환

// 2. String 객체 메소드
// match() : 정규표현식에 일치하는 문자열을 반환
// replace() : 정규표현식에 일치하는 문자열을 다른 문자열로 대체
// search() : 정규표현식에 일치하는 첫 번째 문자열의 인덱스를 반환

// 정규표현식 실습)

// 1) 아이디 : 문자열로 시작하고 문자 또는 숫자 또는 언더스코어(_)만 허용, 최소 8자리 최대 12자리
const strId = 'abcd1234';
const reId = /^[A-Za-z]\w{7,11}$/;
console.log(reId.test(strId));

// 2) 주민번호 : '숫자 6자리 - 숫자 7자리', 앞년도2자리월2자리일2자리 - 성별숫자(1~4)숫자6개
const strSno = '990101-1234567';
const reSno =  /^\d{2}(0[1-9]|1[0-2]])(0[1-9]|1\d|2\d|3[0-1])-[1-4]\d{6}$/;
console.log(reSno.test(strSno));

// 3) 아이피주소 : '숫자 1 ~ 3자리.숫자 1 ~ 3자리.숫자 1 ~ 3자리.숫자 1 ~ 3자리', 숫자는 0 ~ 255로 제한 
