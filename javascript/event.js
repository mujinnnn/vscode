...
//  : 이벤트가 발생하면 이벤트 객체가 이벤트 핸들러(=이벤트 리스너, 이벤트콜백, 콜백)에 
//      인자로 전달이 되서 이벤트 처리를 하는 프로그래밍 모델
// - 자바스크립트가 감지할 수 있는 모든 것은 이벤트
//  : ex) 윈도우가 열림/닫힘, 마우스 누름/뗌/클릭, 키보드 누름/뗌, 스크롤, 데이터 입력, 이미지 로드, 통신(데이터 송/수신), 객체 상태 ...
...
// event handler (=event listener, event callback) : 이벤트 처리를 위한 함수 ex) function(event) {} 또는 event => {}
// event target (=event source) : 이벤트를 발생시킨 객체 ex) <input type="button" ...
// event currentTarget : 현재 이벤트 객체가 전달되어 있는 객체
// event phase (이벤트 단계) : 이벤트 전파 상태
//  (순서 : 캡쳐링 -> 타겟 -> 버블링 순서)
