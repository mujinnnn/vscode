...
console.log(add());                         // NaN (x, y가 undefined)
console.log(add(3));                        // NaN (x는 3, y는 undefined)
...
const func1 = function() {
    console.log("\nfunc1");
}; // 문장이라 ;(세미콜론) 해주는 것이 좋음, 안 써줘도 되긴 함
func1();
...
// 실습 1)
// 함수 리터럴을 이용해서 사칙연산(+, -, *, /)을 수행하는 하나의 함수를 만들어 봅시다.
...
const afunc = (x, y) => {       // 문장이 2개 이상 있을 땐 {}로 묶어준다.
    let result = x + y;
    return result;
};
...
// 실습 3)
// 아래와 같은 함수 하나를 생성
// i) 함수의 파라미터의 수와 함수에 전달된 인자의 수가 같지 않으면 메세지를 출력하고 함수를 실행하지 않는 함수를 생성
// ii) 함수의 파라미터의 수와 전달된 인자의 수가 같다면 모든 인자들을 다 더해서 반환하는 함수
...
// cf) ... 스프레드문법
// 파라미터의 개수를 지정하기 곤란할 때
// 파라미터들을 배열로
const funcl1 = function (...args) {
